...
, int moneda_prod, double cant_falta, double tipo_cambio, double precio_condcto, double precio_politica, string bonificacion, DateTime fecha_factura
        ...
        public bool insertDirrecciones(VW_PUFT_VALI_INSER_DIREC  ObjDir )
        {
            ...
            catch (Exception e)
            {
                ...
                DateTime thisDay = DateTime.Today;
                // Display the date in the default (general) format.
                PUFT_ERRORS error = new PUFT_ERRORS("Class cmdsForm/insertDirrecciones", "ERROR INSERTAR DIRRECION SAP=>MYSQL", e.ToString(), thisDay);
                 ...
                 Int64 id_pedido, Int64 n_entrega)
        {
            ...
            try
            {
                MySqlCommand command = new MySqlCommand("SP_INSERT_FACTURAS_PUFT", conexion);
                command.Connection = conexion;
                command.CommandType = CommandType.StoredProcedure;
                ...
                command.Parameters.AddWithValue("n_entrega", n_entrega);
                if (command.ExecuteNonQuery() == 1)
                {
                    exito = true;
                }
